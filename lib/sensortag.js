...
SensorTag.discover = function(callback, discoveryTimeout) {
  var discoveryTimeout = typeof discoveryTimeout === 'number' ? discoveryTimeout : 5000;
  var discovered = false; onDiscover = function(sensorTag) {
    discovered = true; 
    SensorTag.stopDiscoverAll(onDiscover);
    callback(sensorTag);
  };
  var failedDiscovery = function() {
    if(!discovered) {
      SensorTag.stopDiscoverAll(onDiscover);
      callback();
    }
  };
  setTimeout(failedDiscovery, discoveryTimeout);
